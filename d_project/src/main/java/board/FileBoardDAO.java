...
public class FileBoardDAO {
	...
	public int insertFileBoard(FileBoardDTO board,String id) {
		int insertCount = 0;
		...
		try {
			...
			if(!(rs2.next())) {
				...
				sql = "SELECT MAX(num) FROM file_board";
				pstmt = con.prepareStatement(sql);
...
//최근 게시물 목록 조회
	public ArrayList<FileBoardDTO> selectRecentlyFileBoardList(){
		...
		try {
			con = JdbcUtil.getConnection();
			String sql = "SELECT * FROM file_board ORDER BY readcount desc LIMIT 4";
